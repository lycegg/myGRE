...
public:
    ...
    long long num_inner=0,son_inner=0,num_leaf=0,son_leaf=0;
    void lyc_dfs(node*lyc_cur){
        if(lyc_cur->level==0){
            auto lcur=reinterpret_cast<leaf_node*>(lyc_cur);
            num_leaf+=1;
            son_leaf+=lcur->slotuse;
        }
        else{
            auto icur=reinterpret_cast<inner_node*>(lyc_cur);
            num_inner+=1;
            son_inner+=icur->slotuse;
            for(int i=0;i<=icur->slotuse;i++)
                lyc_dfs(icur->childid[i]);
        }
    }
    void zhuangzailv(){
        num_inner=0,son_inner=0,num_leaf=0,son_leaf=0;
        lyc_dfs(m_root);
        printf("inner%dnodes,%dsons,%d/(%d*%d)=%lf\n",num_inner,son_inner,son_inner,num_inner,innerslotmax,son_inner/(double)(num_inner*innerslotmax));
        printf("leaf%dnodes,%dsons,%d/(%d*%d)=%lf\n",num_leaf,son_leaf,son_leaf,num_leaf,leafslotmax,son_leaf/(double)(num_leaf*leafslotmax));
    }
};

...
public:
    long long cntleaf,cntlevel;
    long long cntslot,cntptr,cntdata,cntmeta;
    void lyc_dfs(Node*node,int level){//参考scan_and_destroy
        cntslot+=node->num_items;
        cntmeta++;
        for (int i = 0; i < node->num_items; i ++) {
            if (BITMAP_GET(node->none_bitmap, i) == 0) {
                if (BITMAP_GET(node->child_bitmap, i) == 0) {
                    ...
                    cntptr++;
                    lyc_dfs(node->items[i].comp.child,level+1);
                }
            }
        }
    }
    void tongji(){
        ...
        lyc_dfs(root,0);
        printf("key_size=%d\n,value_size=%d\n",sizeof(T),sizeof(P));
        long long slotsize=std::max(sizeof(T)+sizeof(P),sizeof(Node*));
        printf("avg_cenggao=%lld/%lld=%lf\n",cntlevel,cntleaf,cntlevel/(double)cntleaf);
        auto sizethis=sizeof(*this)-6*sizeof(cntptr);
        printf("sizeLIPP=%d\n",sizethis);
        printf("sizeNode=%d\n",sizeof(Node));
        printf("cntNode=%lld\n",cntmeta);
        printf("cntslot=%lld\n",cntslot);
        ...
        printf("empty_slot_size=%lld*%lld=%lld\n",slotsize,cntempty,cntempty*slotsize);
    }
};

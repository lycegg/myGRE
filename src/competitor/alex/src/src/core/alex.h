...
public:
  long long cntleafs,cntlevel;
  long long totslot,totson;
  void dfs(AlexNode<T, P>*cur_node,int cur_level){
    if (!cur_node) return;
    if (cur_node->is_leaf_) {
      auto t=static_cast<const data_node_type*>(cur_node);
      /*int cur = 0;
      while (cur < t->num_children_) {
        if(cur==0||t->key_slots_[cur]!=t->key_slots_[cur-1]){
          cntleafs++;
          cntlevel+=t->level_;
        }
        cur++;
      }*/
      cntleafs+=t->num_keys_;
      //cntlevel+=t->num_keys_*t->level_;
      cntlevel+=t->num_keys_*cur_level;
    } else {
      int cur = 0;
      auto cur_inner=static_cast<const model_node_type*>(cur_node);
      totslot+=cur_inner->num_children_;
      while (cur < cur_inner->num_children_) {
        AlexNode<T, P>* child_node = cur_inner->children_[cur];
        dfs(child_node,cur_level+1);
        int repeats = 1 << child_node->duplication_factor_;//lyc:一定2的倍数？。
        totson++;
        cur += repeats;
      }
    }
  ...
  void cenggao_zhuangzailv(){
    ...
    totson=0;
    dfs(root_node_,0);
    printf("avg_cenggao:%lld/%lld=%lf\n",cntlevel,cntleafs,cntlevel/(double)cntleafs);
    printf("inner_zhuangzailv:%lld/%lld=%lf\n",totson,totslot,totson/(double)totslot);
  }
};
}  // namespace alex
